...
class PeerTrust():
    ...
    def minimumTrustTemplate(self, trustorDID, trusteeDID, offerDID):
        ...
        information["trustee"]["trusteeSatisfaction"] = round(
            random.uniform(0.8, 0.95), 4)
        information["trustor"]["trustorDID"] = trustorDID
        information["trustor"]["trusteeDID"] = trusteeDID
        information["trustor"]["offerDID"] = offerDID
        information["trustor"]["credibility"] = round(
            random.uniform(0.75, 0.9), 4)
        information["trustor"]["transactionFactor"] = round(
            random.uniform(0.8, 0.9), 4)
        information["trustor"]["communityFactor"] = round(
            random.uniform(0.85, 0.9), 4)
        information["trustor"]["direct_parameters"]["userSatisfaction"] = round(
            random.uniform(0.5, 0.7), 4)
        ...
        information["trustor"]["direct_parameters"]["interactionNumber"] = self.getInteractionNumber(
            trustorDID, trusteeDID, offerDID)
        information["trustor"]["direct_parameters"]["totalInteractionNumber"] = self.getLastTotalInteractionNumber(
            trusteeDID)
        information["trust_value"] = round(information["trustor"]["direct_parameters"]["direct_weighting"]*(information["trustee"]["trusteeSatisfaction"]*information["trustor"]["credibility"]
                                           * information["trustor"]["transactionFactor"])+information["trustor"]["indirect_parameters"]["recommendation_weighting"]*information["trustor"]["communityFactor"], 4)
        information["currentInteractionNumber"] = self.getCurrentInteractionNumber(
            trustorDID)
    ...
    def minimumTrustValuesDLT(self, producer, consumer_instance, trustor, dict_product_offers):
        ...
        for i in range(4):
            self.list_additional_did_providers.append(rstr.xeger(
                "[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}"))
            additional_did_offers = []
            additional_did_offers.append(rstr.xeger(
                "[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}"))
            additional_did_offers.append(rstr.xeger(
                "[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}"))
                ...
                while (interaction == False):
                    ...
                    if "$" not in aux_did_providers[current_additional_provider] and \
                        ...
                        if "$" not in aux_did_offers[current_additional_provider][current_additional_offer]:
                            ...
                            if "$" not in aux_did_offers[current_additional_provider][current_additional_offer] and \
                                ...
                                aux_did_providers[current_additional_provider] = \
                                    aux_did_providers[current_additional_provider] + "$"
                                current_additional_provider = (
                                    current_additional_provider+1) % len(aux_did_providers)
                    else:
                        current_additional_provider = (
                            current_additional_provider+1) % len(aux_did_providers)
                        if counter >= self.max_different_interactions-1 and \
                            ...
                            if "$" in new_offer:
                                new_offer = self.list_additional_did_offers[
                                    current_additional_provider][current_additional_offer]
                            else:
                                aux_did_offers[current_additional_provider][current_additional_offer] = \
                                    aux_did_offers[current_additional_provider][current_additional_offer] + "$"
                            counter += 1
        ...
        if not bool(self.kafka_interaction_list):
            ...
            for interaction in data:
                trust_informartion = self.minimumTrustTemplate(
                    interaction["trustorDID"], interaction["trusteeDID"], interaction["offerDID"])
                trust_informartion["trustor"]["direct_parameters"]["userSatisfaction"] = round(
                    random.uniform(0.5, 0.75), 4)
                ...
                recommender_list = self.setRecommenderList(
                    trust_informartion["trustor"]["trustorDID"], trust_informartion["trustor"]["trusteeDID"])
    ...
    def setRecommenderList(self, trustorDID, trusteeDID):
        ...
        for aditional_provider in self.list_additional_did_providers:
            if aditional_provider != trustorDID and aditional_provider != trusteeDID:
                trust_value = self.getLastHistoryTrustValue(
                    trustorDID, trusteeDID)
                last_trust_value = self.getLastHistoryTrustValue(
                    aditional_provider, trusteeDID)
                previous_recommendations = self.consumer.readAllRecommenders(
                    self.historical, trustorDID, trusteeDID)
    ...
    def getLastRecommendationValue(self, last_interaction):
        ...
        trust_information = self.consumer.readLastTrustValue(
            self.historical, trustor, trustee)
        if bool(trust_information):
            last_truste_value = trust_information["trust_value"]
        else:
            interaction_list = self.find_by_two_column(
                'trustorDID', trustor, 'trusteeDID', trustee)
            response = requests.post(interaction_list[0]["endpoint"], data=json.dumps(
                interaction_list[0]).encode("utf-8"))
    ...
    def getLastOfferRecommendationValue(self, last_interaction):
        ...
        trust_information = self.consumer.readLastTrustValueOffer(
            self.historical, trustor, trustee, offer)
        if bool(trust_information):
            last_truste_value = trust_information["trust_value"]
        else:
            interaction_list = self.find_by_three_column(
                'trustorDID', trustor, 'trusteeDID', trustee, 'offerDID', offer)
            response = requests.post(interaction_list[0]["endpoint"], data=json.dumps(
                interaction_list[0]).encode("utf-8"))
            response = json.loads(response.text)
            last_truste_value = response['trust_value']
    ...
    def getTrusteeSatisfactionDLT(self, trusteeDID):
        ...
        for interaction in last_interaction:
            ...
            data = {"trustorDID": interaction["trustorDID"],
                    "trusteeDID": interaction["trusteeDID"], "offerDID": interaction["offerDID"]}
            response = requests.post(
                "http://"+endpoint+"/query_satisfaction_value", data=json.dumps(data).encode("utf-8"))
            response = json.loads(response.text)
            general_satisfaction = general_satisfaction + \
                float(response['userSatisfaction'])
            counter = counter + 1
    ...
    def generateHistoryTrustInformation(self, producer, consumer_instance, trustorDID, trusteeDID, offerDID, previous_interaction_number):
        ...
        if previous_interaction_number != 0:
            ...
            information["trustor"]["offerDID"] = offerDID
            information["trustor"]["credibility"] = round(
                random.uniform(0.9, 0.92), 4)
            information["trustor"]["transactionFactor"] = round(
                random.uniform(0.85, 0.88), 4)
            information["trustor"]["communityFactor"] = round(
                random.uniform(0.85, 0.88), 4)
            information["trustor"]["direct_parameters"]["userSatisfaction"] = round(
                random.uniform(0.5, 0.7), 4)
            ...
            for i in range(previous_interaction_number-1):
                interaction_number = self.getInteractionNumber(
                    trustorDID, trusteeDID, offerDID)

                trust_data = self.consumer.readLastTrustInterationValues(
                    self.historical, trustorDID, trusteeDID, offerDID, interaction_number)

                information = trustInformationTemplate.trustTemplate()

                information["trustee"]["trusteeDID"] = trusteeDID
                information["trustee"]["offerDID"] = offerDID
                information["trustee"]["trusteeSatisfaction"] = round((round(random.uniform(
                    0.8, 0.9), 3) + (interaction_number * trust_data["trusteeSatisfaction"]))/(interaction_number+1), 4)
                #information["trustee"]["trusteeSatisfaction"] = round(random.uniform(0.8, 0.9), 3)
                information["trustor"]["trustorDID"] = trustorDID
                information["trustor"]["trusteeDID"] = trusteeDID
                information["trustor"]["offerDID"] = offerDID
                information["trustor"]["credibility"] = round((round(random.uniform(
                    0.8, 0.9), 3) + (interaction_number * trust_data["credibility"]))/(interaction_number+1), 4)
                information["trustor"]["transactionFactor"] = round((round(random.uniform(
                    0.75, 0.95), 3) + (interaction_number * trust_data["transactionFactor"]))/(interaction_number+1), 4)
                information["trustor"]["communityFactor"] = round((round(random.uniform(
                    0.75, 0.9), 3) + (interaction_number * trust_data["communityFactor"]))/(interaction_number+1), 4)
                information["trustor"]["direct_parameters"]["userSatisfaction"] = round(
                    random.uniform(0.5, 0.7), 4)
                direct_weighting = round(random.uniform(0.6, 0.7), 2)
                information["trustor"]["direct_parameters"]["direct_weighting"] = direct_weighting
    ...
    def generateTrusteeInformation(self, producer, consumer, trustorDID, availableAssets, totalAssets, availableAssetLocation, totalAssetLocation, managedViolations, predictedViolations, executedViolations, nonPredictedViolations, consideredOffers, totalOffers, consideredOfferLocation, totalOfferLocation, managedOfferViolations, predictedOfferViolations, executedOfferViolations, nonPredictedOfferViolations):
        ...
        if not bool(self.list_additional_did_providers):
            ...
            self.list_additional_did_offers = [
                []] * self.max_previous_providers_DLT
            """with open(self.dlt_file_name) as f:
                reader = csv.DictReader(f)
                pointer = 0
                for item in reader:"""
            ...
            for item in self.kafka_interaction_list:
                if item["trustorDID"] not in self.list_additional_did_providers and pointer < self.max_previous_interactions_DLT:
                    self.list_additional_did_providers.append(
                        item["trustorDID"])
                pointer += 1
            ...
            for item in self.kafka_interaction_list:
                ...
                if item["trusteeDID"] in self.list_additional_did_providers and item["offerDID"] not in self.list_additional_did_offers:
                    self.list_additional_did_offers[self.list_additional_did_providers.index(
                        item["trusteeDID"])].append(item["offerDID"])
        ...
        information = self.minimumTrustTemplate(
            trustorDID, trusteeDID, offerDID)

        print("\t* Provider ---> "+trusteeDID +
              " -- Product offer ---> "+offerDID)

        information["trustor"]["credibility"] = self.credibility(
            trustorDID, trusteeDID)
        information["trustor"]["transactionFactor"] = self.transactionContextFactor(
            trustorDID, trusteeDID, offerDID)
        information["trustor"]["communityFactor"] = self.communityContextFactor(
            trustorDID, trusteeDID)

        direct_weighting = round(random.uniform(0.6, 0.7), 2)
        information["trustor"]["direct_parameters"]["direct_weighting"] = direct_weighting
        provider_reputation = self.providerReputation(availableAssets, totalAssets, availableAssetLocation,
                                                      totalAssetLocation, managedViolations, predictedViolations, executedViolations, nonPredictedViolations)
        provider_satisfaction = self.providerSatisfaction(
            trustorDID, trusteeDID, provider_reputation, self.consumer)
        offer_reputation = self.offerReputation(consideredOffers, totalOffers, consideredOfferLocation, totalOfferLocation,
                                                managedOfferViolations, predictedOfferViolations, executedOfferViolations, nonPredictedOfferViolations)
        offer_satisfaction = self.offerSatisfaction(
            trustorDID, trusteeDID, offerDID, offer_reputation)
        ...
        information["trustee"]["trusteeDID"] = trusteeDID
        information["trustee"]["offerDID"] = offerDID
        information["trustee"]["trusteeSatisfaction"] = self.getTrusteeSatisfactionDLT(
            trusteeDID)
        information["trustor"]["direct_parameters"]["userSatisfaction"] = self.satisfaction(
            ps_weighting, os_weighting, provider_satisfaction, offer_satisfaction)
        information["trust_value"] = round(information["trustor"]["direct_parameters"]["direct_weighting"]*(information["trustee"]["trusteeSatisfaction"]*information["trustor"]["credibility"]
                                           * information["trustor"]["transactionFactor"])+information["trustor"]["indirect_parameters"]["recommendation_weighting"]*information["trustor"]["communityFactor"], 4)
    ...
    def getLastOfferHistoryTrustValue(self, trustorDID, trusteeDID, offerDID):
        ...
        trust_information = self.consumer.readLastTrustValueOffer(
            self.historical, trustorDID, trusteeDID, offerDID)
    ...
    def getTrustorInteractions(self, trustorDID):
        ...
        list_trustor_interactions = self.find_by_column(
            'trustorDID', trustorDID)
    ...
    def credibility(self, trustorDID, trusteeDID):
        ...
        if previous_trustor_interactions:
            for previous_interaction in previous_trustor_interactions:
                ...
                similarity_summation = similarity_summation + \
                    self.similarity(previous_interaction["trusteeDID"])
        else:
        ...
        credibility = trustee_similarity / \
            (similarity_summation/summation_counter)
        if credibility > 1.0:
            credibility = (similarity_summation /
                           summation_counter)/trustee_similarity
    ...
    def similarity(self, trusteeDID):
        ...
        for interaction in trustor_interaction_list:
            common_interaction = self.getTrusteeInteractions(
                trusteeDID, interaction["trusteeDID"])
        ...
        if bool(trustor_interaction_list) and bool(common_interaction):
            common_interaction_list = self.getTrustorInteractions(
                common_interaction[0]["trusteeDID"])
            ...
            for interaction in trustor_interaction_list:
                if interaction["trusteeDID"] in common_interaction_list:
                    ...
                    trustor_satisfaction_summation = self.consumer.readSatisfactionSummation(
                        self.historical, trusteeDID, interaction["trusteeDID"])
                    if trustor_satisfaction_summation == 0.0:
                        endpoint = interaction["endpoint"].split("/")[2]
                        data = {"trustorDID": trusteeDID,
                                "trusteeDID": interaction["trusteeDID"], "offerDID": None}
                        response = requests.post(
                            "http://"+endpoint+"/query_satisfaction_value", data=json.dumps(data).encode("utf-8"))
                        response = json.loads(response.text)
                        trustor_satisfaction_summation = response['userSatisfaction']

                    common_interaction_satisfaction_summation = self.consumer.readSatisfactionSummation(
                        self.historical, common_interaction[0]["trusteeDID"], interaction["trusteeDID"])
                    if common_interaction_satisfaction_summation == 0.0:
                        endpoint = interaction["endpoint"].split("/")[2]
                        data = {"trustorDID": common_interaction[0]["trusteeDID"],
                                "trusteeDID": interaction["trusteeDID"], "offerDID": None}
                        response = requests.post(
                            "http://"+endpoint+"/query_satisfaction_value", data=json.dumps(data).encode("utf-8"))
                        response = json.loads(response.text)
                    ...
                    satisfaction_summation = pow(
                        (trustor_satisfaction_summation - common_interaction_satisfaction_summation), 2)
                    global_satisfaction_summation = global_satisfaction_summation + satisfaction_summation
                    IJS_counter = IJS_counter + 1

            final_similarity = 1 - \
                math.sqrt(global_satisfaction_summation/IJS_counter)
    ...
    def communityContextFactor(self, trustorDID, trusteeDID):
        """ Static list of recommender based on the domains registered in the DL """
        ...
        total_registered_trustee_interaction = self.consumer.readTrusteeInteractions(
            self.historical, trusteeDID)

        number_trustee_feedbacks_DLT = self.getTrusteeFeedbackNumberDLT(
            trusteeDID)

        trustee_interaction_rate = number_trustee_feedbacks_DLT / \
            total_registered_trustee_interaction

        if trustorDID in trustworthy_recommender_list:
            trustworthy_recommender_list.remove(trustorDID)

        trustworthy_recommendations = self.getTrustworthyRecommendationDLT(
            trustorDID, trusteeDID, trustworthy_recommender_list)
        ...
        for recommender in trustworthy_recommendations:
            ...
            if last_value != 0:
                last_credibility = self.getLastCredibility(
                    trustorDID, recommender)
                summation_trustworthy_recommendations = summation_trustworthy_recommendations + \
                    (last_credibility*last_value)
    ...
    def bad_mouthing_attack_resilience(self, trustorDID, trusteeDID, new_trusteeDID, new_offerDID):
        ...
        total_registered_trustee_interaction = self.consumer.readTrusteeInteractions(
            self.historical, new_trusteeDID)
        ...
        trustworthy_recommendations = self.getTrustworthyRecommendationDLT(
            trusteeDID, new_trusteeDID, self.recommender_list)
        ...
        if len(trustworthy_recommendations) > 0:
            ...
            for recommender in trustworthy_recommendations:
                recommendation_trust = self.consumer.readLastRecommendationTrustValue(
                    self.historical, trustorDID, trusteeDID, recommender)
                if bool(recommendation_trust) and recommendation_trust >= RECOMMENDATION_THRESHOLD:
                    ...
                    print("Recommendation trust, ", recommendation_trust,
                          " is lower than Threshold: ", RECOMMENDATION_THRESHOLD)
            ...
            if not no_recommendations:
                for recommender in trustworthy_recommendations:
                    ...
                    if bool(recommendation_trust) and recommendation_trust >= 0.3:
                        last_trust_score_recommender = self.getLastHistoryTrustValue(
                            trusteeDID, recommender)
                        action_trust = ALPHA_WEIGTHING * last_trust_score_recommender

                        recommendation = self.getLastHistoryTrustValue(
                            recommender, new_trusteeDID)
                        trust_on_recommender = (
                            1-ALPHA_WEIGTHING) * (recommendation_trust * recommendation)

                        recommender_influence = recommendation_trust / average_trust_recommenders
                        summation_trustworthy_recommendations = summation_trustworthy_recommendations + \
                            ((action_trust + trust_on_recommender)
                             * recommender_influence)

                        trustor_template = self.consumer.readAllTemplateTrustValue(
                            self.historical, trustorDID, trusteeDID)
                        ...
                        for recommendation_list in trustor_template["trustor"]["indirect_parameters"]["recommendations"]:
                            if recommendation_list["recommender"] == recommender:
                                recommendation_list["average_recommendations"] = (
                                    (recommendation_list["average_recommendations"] * recommendation_list["recommendation_total_number"]) + recommendation) / (recommendation_list["recommendation_total_number"] + 1)
                        ...
                        if new_recommender:
                            recommendation_list = trustor_template["trustor"][
                                "indirect_parameters"]["recommendations"]
        ...
        if no_recommendations:
            ...
            external_recommendations = self.consumer.start_reading(
                trustorDID, new_offerDID)
            ...
            if bool(external_recommendations):
                ...
                for external_recommendation in external_recommendations:
                    ...
                    summation_trustworthy_recommendations = summation_trustworthy_recommendations + \
                        external_recommendation["trust_value"]
                    "Adding new recommenders"
                    self.recommender_list.append(
                        external_recommendation["trustorDID"])
    ...
    def communityContextFactor2(self, trustorDID, trusteeDID):
        ...
        for recommender in trustworthy_recommendations:
            print("\n\tRecommendation from ", recommender, " over ",
                  trusteeDID, " to calculate the community factor")
            last_value = self.getLastHistoryTrustValue(recommender, trusteeDID)
            print("\tLast trust score of ", recommender,
                  " on ", trusteeDID, " was ---> ", last_value)
            last_credibility = self.getLastCredibility(trustorDID, recommender)
            print("\tCredibility of ", trustorDID, " on the recommender (",
                  recommender, ") --->", round(last_credibility, 4), "\n")
            summation_trustworthy_recommendations = summation_trustworthy_recommendations + \
                (last_credibility*last_value)
    ...
    def transactionContextFactor(self, trustorDID, trusteeDID, offerDID):
        ...
        total_registered_trustee_interaction = self.consumer.readTrusteeInteractions(
            self.historical, trusteeDID)
        total_registered_offer_interactions = self.consumer.readOfferTrusteeInteractions(
            self.historical, trusteeDID, offerDID)

        number_offer_trustee_feedbacks_DLT = self.getOfferFeedbackNumberDLT(
            trusteeDID, offerDID)
        number_trustee_feedbacks_DLT = self.getTrusteeFeedbackNumberDLT(
            trusteeDID)

        try:
            offer_percentage_DLT = number_offer_trustee_feedbacks_DLT / \
                total_registered_offer_interactions
        except ZeroDivisionError:
            offer_percentage_DLT = 0

        try:
            trustee_percentage_DLT = number_trustee_feedbacks_DLT / \
                total_registered_trustee_interaction
        except ZeroDivisionError:
        ...
        if offer_percentage_DLT == 0 and trustee_percentage_DLT != 0:
            transactionFactor = (
                number_trustee_feedbacks_DLT / total_registered_trustee_interaction) / 2
        elif offer_percentage_DLT != 0 and trustee_percentage_DLT == 0:
            transactionFactor = (
                number_offer_trustee_feedbacks_DLT / total_registered_offer_interactions) / 2
        elif offer_percentage_DLT == 0 and trustee_percentage_DLT == 0:
            transactionFactor = 0.5
        else:
            transactionFactor = (number_offer_trustee_feedbacks_DLT / total_registered_offer_interactions +
                                 number_trustee_feedbacks_DLT / total_registered_trustee_interaction)/2
    ...
    def providerSatisfaction(self, trustorDID, trusteeDID, providerReputation, consumer_instance):
        ...
        if not bool(last_interaction):
            ...
            provider_recommendation = self.getLastRecommendationValue(
                last_interaction)

            """ We obtain our last trust value on the recommender from our Kafka topic """
            last_trust_score_recommender = self.getLastHistoryTrustValue(
                trustorDID, last_interaction['trustorDID'])
            ...
            if last_trust_score_recommender == 0:
                provider_satisfaction = round(
                    (providerReputation + provider_recommendation)/2, 4)
            else:
                provider_satisfaction = round(
                    (providerReputation + provider_recommendation * last_trust_score_recommender)/2, 4)
    ...
    def providerReputation(self, availableAssets, totalAssets, availableAssetLocation, totalAssetLocation, managedViolations, predictedViolations, executedViolations, nonPredictedViolations):
        ...
        try:
            violations_percentage = (
                executedViolations + nonPredictedViolations) / predictedViolations
        except ZeroDivisionError:
            violations_percentage = 0

        reputation = ((assets_percentage + assets_location_percentage + (2 *
                      managed_violations_percentage) - (2 * violations_percentage)) + 2) / 6
    ...
    def offerSatisfaction(self, trustorDID, trusteeDID, offerDID, offerReputation):
        ...
        """ Only one recommendation is currently contemplated"""
        last_interaction = self.getRecommenderOfferDLT(
            trustorDID, trusteeDID, offerDID)
        ...
        if not bool(last_interaction):
            ...
            provider_recommendation = self.getLastOfferRecommendationValue(
                last_interaction)

            """ We obtain our last trust value on the offer from our Kafka topic"""
            last_trust_score_recommender = self.getLastOfferHistoryTrustValue(
                last_interaction['trustorDID'], trusteeDID, offerDID)

            """ If we don't have a trust value about the recommender its recommendation is fully contemplated """
            if last_trust_score_recommender == 0:
                provider_satisfaction = round(
                    (offerReputation + provider_recommendation)/2, 4)
            else:
                provider_satisfaction = round(
                    (offerReputation + provider_recommendation * last_trust_score_recommender)/2, 4)
    ...
    def offerReputation(self, consideredOffers, totalOffers, consideredOfferLocation, totalOfferLocation, managedOfferViolations, predictedOfferViolations, executedOfferViolations, nonPredictedOfferViolations):
        ...
        try:
            violations_percentage = (
                executedOfferViolations + nonPredictedOfferViolations) / predictedOfferViolations
        except ZeroDivisionError:
            violations_percentage = 0
